...
public class NoteEditor extends Activity {
    //用于日志记录和调试的标识字符串，值为 "NoteEditor"
    private static final String TAG = "NoteEditor";
    //定义了一个字符串数组，用于创建查询投影，返回笔记的 ID、标题和内容等信息。
    ...
    //用于保存Activity原始状态的标签字符串。
    private static final String ORIGINAL_CONTENT = "origContent";
    // 分别表示编辑和插入两种不同的操作状态，作为常量使用。
    private static final int STATE_EDIT = 0;
    private static final int STATE_INSERT = 1;
    //记录当前Activity的操作状态，可能为STATE_EDIT或STATE_INSERT
    private int mState;
    //用于标识要操作的笔记数据的统一资源标识符（URI）
    private Uri mUri;
    //游标对象，用于从数据提供者获取笔记数据并进行遍历等操作。
    private Cursor mCursor;
    //获取到布局中的EditText视图对象，用于显示和编辑笔记内容。
    private EditText mText;
    //保存笔记的原始内容，用于在需要时恢复数据。
    private String mOriginalContent;


    //显示的每行文本之间绘制线条，以提供一种带有行分隔线的文本编辑效果
    public static class LinedEditText extends EditText {
        //用于定义一个矩形区域，在绘制线条时会基于这个矩形的边界等信息来确定线条的位置等属性。
        private Rect mRect;
        //用于定义绘制线条的画笔属性。
        private Paint mPaint;

        // 构造函数接受一个Context对象和一个AttributeSet对象作为参数，这是在使用布局文件实例化视图时常见的参数形式
        public LinedEditText(Context context, AttributeSet attrs) {
            //首先通过super(context, attrs)调用父类（EditText）的构造函数，以确保EditText的基本初始化工作得以完成，比如设置文本编辑相关的默认属性等。
            super(context, attrs);
            mRect = new Rect();
            mPaint = new Paint();
            //通过创建一个新的Paint对象并设置其样式（setStyle(Paint.Style.STROKE)，
            //这里设置为描边样式，即只绘制线条轮廓而不填充）和颜色（setColor(0x800000FF)，
            //设置为一种特定的蓝色），从而确定了绘制线条时的外观特征。
        ...
        protected void onDraw(Canvas canvas) {
            // 获取当前LinedEditText视图中显示的文本行数
            int count = getLineCount();
            // 获取之前定义的全局Rect和Paint对象，分别赋值给局部变量r和paint
            Rect r = mRect;
            Paint paint = mPaint;
            //遍历每一行文本
            for (int i = 0; i < count; i++) {
                ...
                /*
                 * 在背景上从矩形的左边（r.left）到右边（r.right）绘制一条水平线条，
                 * 并且这条线条的垂直位置是在基线下方 1 个dip（设备独立像素）的地方，
                 * 使用之前设置好属性的paint对象来进行绘制，
                 * 从而实现了在每行文本之间绘制一条特定样式和颜色的线条的效果。
                 */
    ...
    /**
     * 这是Activity生命周期中的关键方法，在Activity首次创建时被调用，
     * 主要负责进行一系列的初始化操作，包括根据启动Activity的Intent设置操作状态、
     * 获取笔记数据的URI、从数据提供者查询笔记数据、
     * 设置Activity的布局以及获取布局中的EditText视图对象等
     */
    @Override
